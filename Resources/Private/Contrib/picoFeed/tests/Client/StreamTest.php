...
class StreamTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testChunkedResponse()
    {
        ...
        self::assertEquals('</rss>', substr($result['body'], -6));
    }
    ...
    public function testDownload()
    {
        ...
        self::assertEquals('text/html; charset=utf-8', $result['headers']['Content-Type']);
        self::assertEquals('<!DOCTYPE html>', substr(trim($result['body']), 0, 15));
        self::assertEquals('</html>', substr(trim($result['body']), -7));
    }
    ...
    public function testRedirect()
    {
        ...
        self::assertEquals(200, $result['status']);
        self::assertEquals('<!DOCTYPE', substr($result['body'], 0, 9));
        self::assertEquals('text/html', $result['headers']['Content-Type']);
        self::assertEquals('http://www.01net.com/editorial/643722/android-google-now-s-ouvre-aux-applications-tierces/#?xtor=RSS-16', $client->getUrl());
    }
    ...
    public function testDecodeGzip()
    {
        if (function_exists('gzdecode')) {
            ...
            self::assertEquals('gzip', $result['headers']['Content-Encoding']);
            self::assertEquals('<!DOC', substr(trim($result['body']), 0, 5));
        }
    }
}
