...
class FilterTest extends PHPUnit_Framework_TestCase
{
    public function testStripHeadTag()
    {
        ...
        self::assertEquals($expected, Filter::stripHeadTags($input));
    ...
    public function testStripXmlTag()
    {
        ...
        self::assertEquals('<rss', substr(Filter::stripXmlTag($data), 0, 4));

        $data = file_get_contents('tests/fixtures/ezrss.it');
        self::assertEquals('<!DOC', substr(Filter::stripXmlTag($data), 0, 5));
        ...
        self::assertEquals('<feed', substr(Filter::stripXmlTag($data), 0, 5));

        $data = file_get_contents('tests/fixtures/atom.xml');
        self::assertEquals('<feed', substr(trim(Filter::stripXmlTag($data)), 0, 5));

        $data = file_get_contents('tests/fixtures/atomsample.xml');
        self::assertEquals('<feed', substr(trim(Filter::stripXmlTag($data)), 0, 5));

        $data = file_get_contents('tests/fixtures/planete-jquery.xml');
        self::assertEquals('<rdf:RDF', trim(substr(trim(Filter::stripXmlTag($data)), 0, 8)));
    }
    ...
    public function testOverrideFilters()
    {
        ...
        $f->setConfig($config);
        self::assertEquals('<p>Testboo</p>', $f->execute());
    }

    public function testNormalizeData()
    {
        // invalid data link escape control character
        self::assertEquals('<xml>random text</xml>', Filter::normalizeData("<xml>random\x10 text</xml>"));
        self::assertEquals('<xml>random text</xml>', Filter::normalizeData('<xml>random&#x10; text</xml>'));
        self::assertEquals('<xml>random text</xml>', Filter::normalizeData('<xml>random&#16; text</xml>'));

        // invalid unit seperator control character (lower and upper case)
        self::assertEquals('<xml>random text</xml>', Filter::normalizeData("<xml>random\x1f text</xml>"));
        self::assertEquals('<xml>random text</xml>', Filter::normalizeData("<xml>random\x1F text</xml>"));
        self::assertEquals('<xml>random text</xml>', Filter::normalizeData('<xml>random&#x1f; text</xml>'));
        self::assertEquals('<xml>random text</xml>', Filter::normalizeData('<xml>random&#x1F; text</xml>'));
        self::assertEquals('<xml>random text</xml>', Filter::normalizeData('<xml>random&#31; text</xml>'));
        ...
        self::assertEquals('<xml>“random“ text</xml>', Filter::normalizeData("<xml>\xe2\x80\x9crandom\xe2\x80\x9c\x08 text</xml>"));
        self::assertEquals('<xml>&#x201C;random&#x201C; text</xml>', Filter::normalizeData('<xml>&#x201C;random&#x201C;&#x08; text</xml>'));
        self::assertEquals('<xml>&#8220;random&#8220; text</xml>', Filter::normalizeData('<xml>&#8220;random&#8220;&#08; text</xml>'));

        // do not convert valid entities to utf-8 character
        self::assertEquals('<xml attribute="&#34;value&#34;">random text</xml>', Filter::normalizeData('<xml attribute="&#34;value&#34;">random text</xml>'));
        self::assertEquals('<xml attribute="&#x22;value&#x22;">random text</xml>', Filter::normalizeData('<xml attribute="&#x22;value&#x22;">random text</xml>'));
    }
}
